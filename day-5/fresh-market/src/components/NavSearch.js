      ...
      <div className="w-1/4 flex flex-row">
        <div className="flex flex-row self-center w-20 ml-auto">
          ...
          <span class="relative inline-block">
            ...
            {cart && cart.items && (<span class="absolute top-0 right-0 inline-flex items-center justify-center px-2 py-1 text-xs font-bold leading-none text-red-100 transform translate-x-1/2 -translate-y-1/2 bg-red-600 rounded-full">{cart.items.length}</span>)}
          </span>
        </div>
      </div>
    </div>
